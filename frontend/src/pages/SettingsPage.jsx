...
const SettingsPage = () => {
  ...
  useEffect(() => {
    if (user?.config) {
      setForm({
        streaming: !!user.config.streaming,
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await updateMe({
        ...
        max_token_limit: form.max_token_limit ? Number(form.max_token_limit) : null,
  ...
  if (!user) {
    return <div className="flex justify-center items-center h-60"><Loader2 size={24} className="animate-spin text-indigo-400" /></div>;
  }
      ...
      <div className="max-w-xl mx-auto px-6 py-8 space-y-8">
        ...
        <div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 divide-y divide-gray-800">
            ...
            <div className="py-4 flex justify-between">
              ...
              <span className="text-sm text-indigo-400 font-mono">{user.tokens_remaining?.toLocaleString() ?? '—'}</span>
            </div>
            <div className="py-4 flex justify-between">
              <span className="text-sm text-gray-400">Total tokens used</span>
              <span className="text-sm text-gray-300 font-mono">{user.total_tokens_used?.toLocaleString() ?? '0'}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-5">
            <Field label="Enable Streaming" hint="Stream AI responses token by token in real time. Matches VITE_ENABLE_STREAMING env.">
              <Switch
            ...
            <Field label="Max Token Limit" hint="Cap the number of tokens the AI can generate per response. Leave blank for default.">
              <input
                type="number"
                min="100"
                max="32000"
                value={form.max_token_limit}
                onChange={(e) => setForm({ ...form, max_token_limit: e.target.value })}
                placeholder="Default"
            ...
            <Field label="Chat Character Limit" hint="Maximum number of characters per message you can send.">
              <input
                type="number"
                min="100"
                max="10000"
                value={form.chat_char_limit}
                onChange={(e) => setForm({ ...form, chat_char_limit: e.target.value })}
                className="w-28 px-3 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm text-right focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </Field>
          </div>
