...
import { UploadCloud, Link2, GitBranch, FileText, Globe, Loader2, Trash2, RefreshCw } from 'lucide-react';
import {
  ingestFile,
  ingestUrl,
  ingestGithub,
...
const IngestPage = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const updated = await Promise.all(
        jobs.map(async (job) => {
          try {
            const status = await pollStatus(job.task_id);
      ...
      setJobs(updated);
      const completed = updated.filter((j) => j.status === 'completed' || j.status === 'failed');
      if (completed.length > 0) {
        completed.forEach((j) => {
          if (j.status === 'completed') toast.success(`"${j.source}" ingested successfully.`);
  ...
  const tabItems = [
    {
      key: 'file',
      label: <span className="flex items-center gap-1.5"><UploadCloud size={14} /> PDF</span>,
      children: (
        <div className="space-y-4">
          <p className="text-xs text-gray-400">Upload a PDF file to ingest into the knowledge base.</p>
          <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-700 rounded-xl cursor-pointer hover:border-indigo-500/60 transition-colors bg-gray-900/40">
            ...
            <input type="file" accept=".pdf" className="hidden" onChange={handleFileUpload} disabled={submitting} />
          </label>
          {publicSwitch}
        </div>
      ),
    ...
    {
      key: 'url',
      label: <span className="flex items-center gap-1.5"><Link2 size={14} /> URL</span>,
      children: (
        <div className="space-y-4">
          <p className="text-xs text-gray-400">Ingest content from a web page URL.</p>
    ...
    {
      key: 'github',
      label: <span className="flex items-center gap-1.5"><GitBranch size={14} /> GitHub</span>,
      children: (
        <div className="space-y-4">
          <p className="text-xs text-gray-400">Ingest code from a GitHub repository.</p>
          <div className="flex gap-2">
            <input
              value={repoInput}
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm rounded-lg transition-colors flex items-center gap-1.5"
            >
              {submitting ? <Loader2 size={14} className="animate-spin" /> : <GitBranch size={14} />}
              Ingest
            </button>
          </div>
          {publicSwitch}
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <div className="border-b border-gray-800 px-6 py-4 shrink-0">
        ...
        <p className="text-xs text-gray-500 mt-0.5">Add documents to the knowledge base</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {jobs.length > 0 && (
          <div className="space-y-2">
            ...
            {jobs.map((job) => (
              <div key={job.task_id} className="flex items-center justify-between bg-gray-900 border border-gray-800 rounded-lg px-4 py-2.5">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-medium text-gray-400">Documents ({docs.length})</h3>
          ...
          {loadingDocs ? (
            ...
            <div className="space-y-2">
              {docs.map((doc) => (
                <div key={doc.document_id} className="flex items-center gap-3 bg-gray-900 border border-gray-800 rounded-lg px-4 py-3 group">
                  {doc.type === 'web' ? <Globe size={14} className="text-blue-400 shrink-0" /> : doc.type === 'github' ? <GitBranch size={14} className="text-gray-300 shrink-0" /> : <FileText size={14} className="text-orange-400 shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white truncate">{doc.source}</p>
                    <p className="text-xs text-gray-500">{doc.type} · {doc.chunk_count ?? '?'} chunks {doc.is_public ? '· Public' : '· Private'}</p>
                  </div>
                  <button
